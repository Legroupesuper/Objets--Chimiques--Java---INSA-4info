...
public final class Solution implements Collection<Object>{
	 ...
	 * @param reactionRuleObject The reaction rule added in the solution.
	 * @return the reactionRule if valid, else a corrected reactionRule, else null (if correction is impossible)
	 * @see Dontreact
	 * @see ReactionRule
	 */
	private Object checkReactionRuleReactive(ReactionRule reactionRuleObject) {
		...
		if(!classOK){
			...
			if(!_threadTable.containsKey(reactionRuleObject)){
				ChemicalThread t = new ChemicalThread(reactionRuleObject, this, _threadGroup);
				_threadTable.put(reactionRuleObject, t);
...
//				addElement = checkReactionRuleReactive((ReactionRule)newReactive);
			...
			if(!className.equals(SubSolutionReactivesAccessor.class.getName()) && target != null){
				...
				if(_mapElements.get(rawClassName) != null){
					result = _mapElements.get(rawClassName).add(target);
				}
